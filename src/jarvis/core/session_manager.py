...
class SessionManager:
    ...
    def detect_context_reference(self, user_input: str) -> bool:
        ...
        has_strong_pronoun = any(
            re.search(pattern, text_lower) for pattern in strong_pronouns
        )

        # Temporal + action combo (e.g., "now show", "again do")
        temporal_markers = [r"\bnow\b", r"\bjust\b", r"\bagain\b", r"\bonce\s+more\b"]
        has_temporal = any(
            re.search(pattern, text_lower) for pattern in temporal_markers
        )

        # Reference words
        reference_words = [r"\bsame\b", r"\bprevious\b", r"\bprior\b", r"\blast\b"]
        has_reference = any(
            re.search(pattern, text_lower) for pattern in reference_words
        )
        ...
        has_action_phrase = any(
            re.search(pattern, text_lower) for pattern in action_phrases
        )
    ...
    def _is_semantically_related(self, current_input: str, previous_input: str) -> bool:
        ...
        overlap_ratio = len(overlap) / min(
            len(current_keywords), len(previous_keywords)
        )
