...
from .utils.syntax_output import (
    print_command_output,
    print_error_output,
    print_inline_command,
    print_syntax,
)
...
try:
    ...
    BrowserManager = None  # type: ignore[misc,assignment]
...
# Setup AI
api_key = (
    config_mgr.config.google_api_key
    or config_mgr.config.api_key
    or os.getenv("NEXUS_API_KEY")
    or os.getenv("GOOGLE_API_KEY")
    or os.getenv("JARVIS_API_KEY")
)
openrouter_key = (
    config_mgr.config.openrouter_api_key
    or os.getenv("NEXUS_OPENROUTER_API_KEY")
    or os.getenv("OPENROUTER_API_KEY")
    or os.getenv("JARVIS_OPENROUTER_API_KEY")
)
groq_key = (
    config_mgr.config.groq_api_key
    or os.getenv("NEXUS_GROQ_API_KEY")
    or os.getenv("GROQ_API_KEY")
    or os.getenv("JARVIS_GROQ_API_KEY")
)
groq_gpt_key = (
    config_mgr.config.groq_gpt_api_key
    or os.getenv("NEXUS_GROQ_GPT_API_KEY")
...
if not llm_client:
    ...
    fallback_clients.append(llm_client)
    console.print(
        "[bold yellow]⚠ Running with reduced intelligence. Provide API keys for a better experience.[/bold yellow]"
    )
...
def do(request: str):
    ...
    if isinstance(llm_client, MockLLMClient):
        console.print(
            "[yellow]No API Key found. Using mock mode. Set Keys to use real AI.[/yellow]"
        )
    ...
    if (
        ...
        and hasattr(llm_client, "memory_client")
        and llm_client.memory_client
    ):
...
def browse(
    ...
    if not browser_manager:
        console.print(
            "[bold red]Error:[/bold red] JARVIS_API_KEY is not set. Please add it to your .env file."
        )
        return
    ...
    console.print(
        Panel(
            f"[bold blue]Browser Task:[/bold blue] {task}\n[bold yellow]Mode:[/bold yellow] {mode}",
            title="Browsing",
        )
    )
...
def search(query: str):
    ...
    if not isinstance(llm_client, GoogleGenAIClient):
        console.print(
            "[red]Search is only supported with Google (Gemini) provider.[/red]"
        )
        return

    if llm_client is None:
        console.print(
            "[bold red]Error:[/bold red] Search requires an active AI client."
        )
        return

    console.print(
        Panel(f"[bold blue]Query:[/bold blue] {query}", title="Google Search")
    )
...
def find(query: str):
    ...
    console.print(
        Panel(f"[bold blue]Searching for:[/bold blue] {query}", title="Local Search")
    )
    ...
    if rc == 0:
        console.print(
            Panel(
                out.strip()
                if out.strip()
                else "[yellow]No matches found anywhere.[/yellow]",
    ...
    if ctx.invoked_subcommand is None:
        ...
        def apply_heuristics(text: str) -> Intent | None:
            ...
            if text.startswith("find file") or text.startswith("search file"):
                ...
                return Intent(
                    action="COMMAND", command="/find", args=query, confidence=1.0
                )

            if text.startswith("read file") or text.startswith("cat "):
                path = text.replace("read file", "").replace("cat ", "").strip()
                return Intent(
                    action="COMMAND", command="/read", args=path, confidence=1.0
                )
            return None
