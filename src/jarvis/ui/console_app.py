...
class JarvisApp:
    def __init__(self, llm_client=None, video_manager=None, browser_manager=None, executor=None):
        ...
        self.style = PromptStyle.from_dict({
            'prompt': '#ff8800 bold',  # Orange prompt like Claude's theme
            'input': '#ffffff',
        })
    ...
    def print_header(self):
        ...
        # ASCII Art Logo
        title_font = pyfiglet.figlet_format("JARVIS", font="slant")
        title_text = Text(title_font, style="bold orange1")
        
        # Metadata
        meta_text = Text()
        meta_text.append("v2.1.0 · Claude 3.5 Sonnet · API Usage Billing\n", style="dim white")
        meta_text.append("~/Jarvis Code", style="dim blue")
    ...
    async def handle_chat(self, text: str):
        ...
        with self.console.status("[bold orange1]Jarvis is thinking...[/bold orange1]", spinner="dots"):
            try:
                loop = asyncio.get_event_loop()
