...
class OnboardingUI:
    ...
    def run(self):
        ...
        self.console.print("[dim]Nexus needs API keys to function. Your keys are stored locally.[/dim]\n")
        
        # Google (Required)
        self.console.print("[bold cyan]1. Google Gemini API Key[/bold cyan] [red](Required)[/red]")
        ...
        while not google_key:
             self.console.print("[red]Google API Key is required for search functions.[/red]")
             google_key = Prompt.ask("Enter Google API Key", password=True)
        ...
        while not openrouter_key:
             self.console.print("[red]OpenRouter API Key is required for core intelligence.[/red]")
             openrouter_key = Prompt.ask("Enter OpenRouter API Key", password=True)
        ...
        if has_system_memory:
             self.console.print(f"[green]✓ System Memory Key Detected[/green]")
             use_memory = Confirm.ask("Enable [bold]Nexus Memory[/bold] (Context Intelligence)?", default=True)
        else:
             self.console.print("[dim]System Memory Key not found in environment. Memory features disabled.[/dim]")
             use_memory = False
        ...
        sleep(1)
        self.console.print("[bold green]Configuration saved![/bold green] Initializing Nexus...\n")
        sleep(1)
        self.console.clear()
...
*   **Web Integration**: Search and interact with the web.
        ...
        self.console.print(Panel(Markdown(welcome_text), title="[bold blue]System Initialization[/bold blue]", border_style="blue"))
